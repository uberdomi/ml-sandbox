...
class RescaleTransform:
    ...
    def __call__(self, image):
        ...
        # Iterate over multidimensional indices of the array and change its values in-place
        for idx in np.ndindex(ret_image.shape):
            entry = ret_image[idx]
            ret_image[idx] = self.min + self.max*(entry-self._data_min)/(self._data_max-self._data_min)
