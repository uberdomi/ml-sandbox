...
class Solver(object):
    ...
    def _step(self):
        ...
        y_out = model(X_train)  # Forward pass to get predictions
        # loss = loss_func(y_out, y_train)  # Compute the forward loss
        dout = loss_func.backward(y_out, y_train)  # Loss backward gradient to pass to the backward pass
        grad = model.backward(dout)  # Backward pass, gradient w.r.t. the model parameters
        opt.step(grad)  # Perform the optimization step, updates the weights of the model
