...
class Classifier(Network):
    ...
    def forward(self, X):
        ...
        s = np.dot(X, self.W)  # Affine transformation, dimensions: (N, D+1) x (D+1, 1) = (N, 1)
        z = self.sigmoid(s)  # Sigmoid activation, dimensions: (N, 1)
        self.cache = (X, s, z)
    ...
    def backward(self, dout):
        ...
        dW = None # dL/dW = dL/dy * dy/ds * ds/dw, but we transpose everything cause why not
        # ds/dw = X
        # dy/ds = z*(z-1) - should be diagonal but for the operations we can just use the vectorized form
        # dL/dy = dout.T
        # So everything together: we calculate dW = X.t @ (z * (1 - z)) * dout
        ...
        dyds = z * (1 - z)
        dW = np.dot(X.T, (dout * dyds))  # (D, N) @ (N, M) = (D, M); dout * dyds scales ROWS of dout, retains the shape (N, M)
        
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
        return dW # (dL/dW); everything transposed as to the regular approach
